...
def generate_matrix():
    def f(X1, X2, X3):
        ...
        y = 6.8 + 1.7 * X1 + 5.7 * X2 + 9.9 * X3 + 8.9 * X1 * X1 + 0.5 * X2 * X2 + 6.1 * X3 * X3 + 9.9 * X1 * X2 + \
            0.7 * X1 * X3 + 3.2 * X2 * X3 + 8.4 * X1 * X2 * X3 + randrange(0, 10) - 5
        return y
...
def run_experiment():
    ...
    while not adekvat:
        ...
        if d == 9:
            print("Кількість незначимих коефіцієнтів = 2, приймаємо, що модель неадекватна")
            break
        else:
            if fisher_test():
